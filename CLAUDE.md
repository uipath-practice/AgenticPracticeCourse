...
- English is the **master** language. Site builds English at `/`, translations at `/zh/` and `/ko/` (untranslated pages fall back to English).
- Translations sit next to their source: `page.md` → `page.zh.md`, `page.ko.md`. **Never hand-edit translation files** — they're generated by `/sync-translations` from the English source.
- Everything the learner types or pastes (all code block contents), UI labels, and platform names stay English in every language. Screenshots are never localized.
- After editing any published English page: run `python3 scripts/translation_status.py`, then `/sync-translations`. Drafts are translated at publish time.
- Full rules and glossary: `Master/Localization.md`.
